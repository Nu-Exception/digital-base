...
const handlers = createResourceHandlers({
  ...
  required: ["title", "url"],
  order: "category ASC, sort_order ASC, id DESC"
});

async function getColumns(env) {
  const { results } = await env.DB.prepare("PRAGMA table_info(bookmarks)").all();
...
function buildOrder(columns) {
  const parts = [];
  if (columns.includes("category")) parts.push("category ASC");
...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const table = await env.DB.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'bookmarks'").first();
    const table_exists = Boolean(table);
    if (!table_exists) return debug ? json({ table_exists, columns: [], total: 0, recent: [] }) : json({ bookmarks: [] });

    const columns = await getColumns(env);
    const order = buildOrder(columns);
    const total = await env.DB.prepare("SELECT COUNT(*) AS count FROM bookmarks").first();
    if (debug) {
      const recent = await env.DB.prepare(`SELECT * FROM bookmarks ORDER BY ${order} LIMIT 5`).all();
      return json({ table_exists, columns, total: total?.count ?? 0, recent: recent.results || [] });
    }

    const where = admin ? "" : buildWhere(columns);
    const { results } = await env.DB.prepare(`SELECT * FROM bookmarks ${where} ORDER BY ${order}`).all();
